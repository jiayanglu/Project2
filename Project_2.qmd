...
fun_summary1 <- function(start_date, end_date){
  ...
  #return(type_of_report_animal_year_range_stat)
...
paste0("Summary statistics for percentage of types of adverse events from ", start_date, " to ", end_date, " queried from openFDA <animalandveterinary> endpoint.\n")
return(fun_summary1(start_date, end_date))
...
fun_summary2 <- function(start_date, end_date){
  ...
  #return(outcomes_animal_year_range_stat)
...
server <- function(input, output, session) {
  ...
  output$plot_reports_over_time <- renderPlot({
    ...
    output$plot_types_of_report  <- renderPlot({
      ...
      if (input$endpoint == "animalandveterinary" && input$type_of_data == "Types of report") {
        plot <- fun_plot3(input$endpoint, input$start_date, input$end_date)
        return(plot)
      } else if (input$endpoint == "drug" && input$type_of_data == "Types of report"){
    ...
    output$plot_outcomes <- renderPlot({
      ...
      if (input$endpoint == "animalandveterinary" && input$type_of_data == "Outcomes") {
        plot <- fun_plot4(input$endpoint, input$start_date, input$end_date)
        return(plot)
      } else if (input$endpoint == "drug" && input$type_of_data == "Outcomes"){
        validate(need(FALSE, "Error: Plot is only available for 'Reports over time' and 'Who reports' tabs in 'drug' endpoint."))
        return(NULL)
      }
    })
  ...
  tabsetPanel(
    ...
    tabPanel("About",
             fluidRow(
               column(6,
                      h2("About This App"),
                      tags$p("This app provides analysis of adverse events reports."),
                      tags$p("It allows users to query data from an API, visualize data summaries, and download data subsets."),
                      tags$p("For more information about the data source, please visit ",
                             tags$a("Data Source Website", href = "https://example.com"),
                             "."),
                      tags$p("Purpose of Each Tab:"),
                      tags$ul(
                        tags$li("About: Provides information about the app and its purpose."),
                        tags$li("Data Download: Allows users to query and download data."),
                        tags$li("Data Exploration: Enables exploration of data through plots and summaries.")
                      )
               ),
               column(6,
                      h2("Related Image"),
                      img(src = "path_to_your_image.png", height = 200, width = 200)
               )
             )
    ),
...
output$exploration_table <- renderUI({
  ...
  if (!is.null(table_data)) {
    ...
    table_ui <- fluidRow(
      column(12,
             tableOutput("exploration_table_output")
      )
    )
